...
local pov215 = 13.7069 
local pov365 = 45.2669
local pov685 = 82.2031 

/* from DLW
levelnote	cpi2017	icp2017
rural	1.216772	17.09197
urban	1.2663806	20.787346
*/
local cpi2017rur = 1.216772 
local icp2017rur = 17.09197
local cpi2017urb = 1.2663806
local icp2017urb = 20.787346
...
****************************************************   
//Load microdata			
*dlw, country(ind) year(2021) type(gmd) files mod(gpwg)
use "${upath2}\02.input\IND\IND_2021_CPHS_V01_M_V02_A_GMD_GPWG.dta" , clear
merge m:1 hhid using "${upath2}\02.input\IND\CPHS_2021_HHID_state.dta" 
drop if _merge==2
drop _merge
decode state, gen(subnatid)

levelsof sim, local(simlist)
tempfile datax
save `datax', replace

qui foreach simout of local simlist {
	use `datax', clear
	keep if sim==`simout'
	ren sim simout
	noi dis "doing imputed welfare - `simout'"
	merge 1:m hhid using "${upath2}\02.input\IND\IND_2021_ALL.dta" 
	drop if _merge==2
	drop _merge
	
	//Telangana updated
	replace subnatid = "Andhra Pradesh" if subnatid=="Telangana"
	...
	// welfare variable     		
	gen gallT_ppp = welfare/`cpi2017urb'/`icp2017urb'/365    if urban==1
	replace gallT_ppp = welfare/`cpi2017rur'/`icp2017rur'/365    if urban==0
	replace gallT_ppp = 0.25 if gallT_ppp<0.25
	
	//ind weight
	bys hhid (pid): gen hx = _N
	gen double weight_p = weight/hx
	...
	****************************************************   
	...
	//loop through random assignments
		...
		qui forv sim=1(1)$sim {				
			...
			*/	
			//ASPIRE IND = 100, June 15 version
			gen dep_sp = 0
		...
		gen simout = `simout'
		order code survname level sample baseyear lineupyear mean_ln poor215_ln poor685_ln dep_* multvul_* all*vul* pov1* dim6* total* sh_* nohh noind
		saveold "${upath2}\03.intermediate\Sim\2021\temp\IND_2021_CPHS_2021_`simout'", replace
...
groupfunction, mean(poor* multvul_* all6vul_* all5vul_* all4vul_* all3vul_* all2vul_* dep_* pov1_* dim6* mean_ln total* sh_* nohh noind) by(code baseyear lineupyear survname level sample)
gen todo = 1
saveold "${upath2}\03.intermediate\Sim\2021\IND_2021_CPHS_2021", replace
