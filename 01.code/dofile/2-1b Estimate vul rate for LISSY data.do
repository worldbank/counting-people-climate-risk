...
qui forv i=1(1)`all' { 
		...
		****************************************************   
		...
		collapse (mean) welfare  poor* multvul_* all6vul_* all5vul_* all4vul_* all3vul_* all2vul_* dep_* p1_* dim6* (rawsum) _count h [aw=hpopwgt], by(file year reg_rural) 
			 
		gen data_`x' = 1
	
	global reglvl reg_rural
		
	//regional level tables 
	noi dis "`code'-`year' - `x'" 
